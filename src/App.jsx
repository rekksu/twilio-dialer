...
const PHONE_CONFIG_URL  = "https://us-central1-vertexifycx-orbit.cloudfunctions.net/getPhoneConfig";
...
export default function OrbitPhone() {
  ...
  // ✅ For OUTBOUND only: fetch recording flag from getPhoneConfig before the call starts.
  // <Parameter> only works inside <Client> (inbound), NOT inside <Dial><Number> (outbound PSTN).
  useEffect(() => {
    if (!isOutbound || !fromNumber) return;
    const fetchConfig = async () => {
      try {
        const res  = await fetch(`${PHONE_CONFIG_URL}?from=${encodeURIComponent(fromNumber)}`);
        const data = await res.json();
        if (data.recording === true) {
          setIsRecording(true);
          console.log("🔴 Outbound recording enabled for", fromNumber);
        }
      } catch (err) {
        console.warn("Could not fetch phone config:", err);
      }
    };
    fetchConfig();
  ...
  useEffect(() => {
    let t;
    if (inCall && !onHold) t = setInterval(() => setCallDuration((p) => p + 1), 1000);
  ...
  const resetCall = () => {
    setIncoming(false); setInCall(false); setMicMuted(false);
    setOnHold(false); setShowKeypad(false); setIsRecording(false); setCallDuration(0);
  ...
  useEffect(() => {
    const verify = async () => {
      ...
      try {
        ...
        setAuthorized(true);
      } catch { setAuthorized(false); } finally { setAuthChecked(true); }
    };
    verify();
  }, [accessKey]);
  ...
  useEffect(() => {
    const init = async () => {
      ...
      try {
        ...
        const { token } = await (await fetch(`${TOKEN_URL}?identity=${agentId}`)).json();
        ...
        device.on("incoming", (call) => {
          ...
          call.on("accept", () => {
            ...
            // ✅ INBOUND: <Parameter> inside <Client> IS valid — read it here
            const flag = call.customParameters?.get("recording");
            if (flag === "true") { setIsRecording(true); console.log("🔴 Inbound recording active"); }
          });
          call.on("disconnect", () => { resetCall(); setPhoneNumber(""); setStatus("Call ended");   setTimeout(() => setStatus("Ready"), 2000); });
          call.on("cancel",     () => { resetCall(); setPhoneNumber(""); setStatus("Missed call");   setTimeout(() => setStatus("Ready"), 2000); });
          call.on("reject",     () => { resetCall(); setPhoneNumber(""); setStatus("Call rejected"); setTimeout(() => setStatus("Ready"), 2000); });
          call.on("error",  (e) => { resetCall(); setPhoneNumber(""); setStatus(`Error: ${e.message}`); setTimeout(() => setStatus("Ready"), 2000); });
        });
  ...
  const makeOutbound = async (number = phoneNumber) => {
    if (!deviceRef.current || !number) { setStatus(!deviceRef.current ? "Device not ready" : "Enter a number"); return; }
    setStatus(`Calling ${number}...`);
    try {
      const call = await deviceRef.current.connect({ params: { To: number, From: fromNumber || "+1234567890" } });
      callRef.current = call;
      setInCall(true);
      // ✅ OUTBOUND: recording flag already set by getPhoneConfig useEffect above
      call.on("ringing",    () => setStatus("Ringing..."));
      call.on("accept",     () => setStatus("Connected"));
      call.on("disconnect", () => { resetCall(); setPhoneNumber(""); setStatus("Call ended"); if (isOutbound) setTimeout(() => window.close(), 1000); });
      call.on("error",  (e) => { setStatus(`Call failed: ${e.message}`); resetCall(); });
    } catch (e) { setStatus(`Connection failed: ${e.message}`); resetCall(); }
  };

  const accept = () => { if (!callRef.current) return; callRef.current.accept(); setIncoming(false); setInCall(true); setStatus("Connected"); };
  const reject = () => { if (!callRef.current) return; callRef.current.reject(); setIncoming(false); setInCall(false); setPhoneNumber(""); setStatus("Call rejected"); };
  const hangup = () => { if (!callRef.current) return; callRef.current.disconnect(); resetCall(); };
  const toggleMic = () => { if (!callRef.current) return; callRef.current.mute(!micMuted); setMicMuted(!micMuted); };
  const toggleKeypad = () => setShowKeypad((p) => !p);
  ...
  const toggleHold = () => {
    ...
    if (next) {
      ...
      try { callRef.current.sendDigits("*"); } catch (_) {}
  ...
  if (!authChecked) return <Screen><div style={s.centerContent}><div style={s.loader}/><p style={s.statusText}>Verifying access...</p></div></Screen>;
  if (!authorized)  return (
    <Screen><div style={s.centerContent}>
      ...
      <div style={s.phone}>
        <div style={s.header}>
          <div style={s.headerContent}>
            ...
            <div style={s.statusBadge}><div style={s.statusDot}/><span style={s.statusLabel}>Online</span></div>
          </div>
        </div>
          ...
          {incoming && (
            <div style={s.incomingContainer}>
              <div style={s.callerInfo}>
                <div style={s.avatarRing}><div style={s.avatar}>
                ...
                <div style={s.callerDetails}>
                  ...
                  <div style={s.callerNumber}>{fmt(phoneNumber)}</div>
                </div>
              </div>
            ...
            <div style={s.activeCallContainer}>
              <div style={s.activeCallInfo}>
                ...
                <div style={s.activeCallDetails}>
                  <div style={s.activeNumber}>{fmt(phoneNumber)}</div>
              ...
              <div style={s.callControls}>
                <button style={{...s.controlBtn,...(micMuted&&!onHold?s.controlBtnActive:{})}} onClick={toggleMic} disabled={onHold}>
                  <div style={s.controlIconContainer}>
                    {micMuted
                      ? <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><line x1="1" y1="1" x2="23" y2="23"/><path d="M9 9v3a3 3 0 0 0 5.12 2.12M15 9.34V4a3 3 0 0 0-5.94-.6"/><path d="M17 16.95A7 7 0 0 1 5 12v-2m14 0v2a7 7 0 0 1-.11 1.23"/><line x1="12" y1="19" x2="12" y2="23"/><line x1="8" y1="23" x2="16" y2="23"/></svg>
              ...
              <div style={s.secondaryControls}>
                <button style={{...s.secondaryControlBtn,...(showKeypad?s.secondaryControlBtnActive:{})}} onClick={toggleKeypad}>
...
const s = {
  page:{minHeight:"100vh",width:"100vw",display:"flex",justifyContent:"center",alignItems:"center",background:"linear-gradient(135deg,#667eea 0%,#764ba2 100%)",fontFamily:"-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif",padding:"20px"},
  phone:{width:420,maxWidth:"100%",background:"#fff",borderRadius:32,boxShadow:"0 25px 80px rgba(0,0,0,0.25),0 10px 40px rgba(0,0,0,0.15)",overflow:"hidden",display:"flex",flexDirection:"column",position:"relative"},
  ...
  activeCallDetails:{display:"flex",flexDirection:"column",gap:8,alignItems:"center"},
  ...
  recordingDot:{width:8,height:8,borderRadius:"50%",background:"#ef4444",flexShrink:0,animation:"recordingPulse 1.5s ease-in-out infinite"},
