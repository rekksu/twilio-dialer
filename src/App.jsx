...
export default function OrbitPhone() {
  ...
  // ✅ THE KEY FUNCTION:
  // Your Cloud Functions already embed recording="true"/"false" as a
  // custom <Parameter> on every call. This just reads it from the call object.
  // No Firebase SDK, no extra fetch, no getPhoneConfig needed at all.
  const applyRecordingFlag = (call) => {
    const flag = call.customParameters?.get("recording");
    const on   = flag === "true";
    setIsRecording(on);
    console.log(`🎙️ Recording param from CF: "${flag}" → isRecording: ${on}`);
  };
      ...
      try {
        ...
        device.on("incoming", (call) => {
          ...
          call.on("accept", () => {
            ...
            applyRecordingFlag(call); // ✅ reads from your inboundCall CF param
          });
  ...
  const makeOutbound = async (number = phoneNumber) => {
    ...
    try {
      ...
      call.on("accept",     () => { setStatus("Connected"); applyRecordingFlag(call); }); // ✅ reads from outboundCall CF param
